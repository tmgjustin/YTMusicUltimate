...
struct ReleaseDetailView: View {
    ...
    var body: some View {
        ScrollView {
            if isLoading {
                ProgressView("載入中…").frame(maxWidth: .infinity, minHeight: 300)
            } else if let msg = errorMessage {
                ContentUnavailableView(msg, systemImage: "exclamationmark.triangle").frame(minHeight: 300)
            } else if let detail {
        ...
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                    if let year = detail.year { MetaCell(icon: "calendar", label: "年份", value: "\(year)") }
                    if let country = detail.country { MetaCell(icon: "globe", label: "國家", value: country) }
                ...
                if let price = detail.lowestPrice, let num = detail.numForSale {
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(String(format: "US$ %.2f", price))
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 0) {
                ForEach(Array(tracks.enumerated()), id: \.offset) { idx, track in
                    HStack(spacing: 10) {
                        Text(track.position).font(.caption.monospaced()).foregroundStyle(.secondary)
...
struct FlowLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        ...
        let h = rows.map(\.maxHeight).reduce(0, +) + spacing * CGFloat(max(0, rows.count - 1))
    ...
    private func layoutRows(proposal: ProposedViewSize, subviews: Subviews) -> [Row] {
        ...
        var rows: [Row] = []; var current = Row(); var usedW: CGFloat = 0
        for sv in subviews {
            let sz = sv.sizeThatFits(ProposedViewSize(width: maxW, height: nil))
            if usedW + sz.width > maxW, !current.items.isEmpty { rows.append(current); current = Row(); usedW = 0 }
            current.items.append((sv, sz)); usedW += sz.width + spacing
        }
