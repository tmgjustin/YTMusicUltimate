...
struct CameraScanView: View {
    ...
    var body: some View {
        ...
        SimulatorPickerView(viewModel: viewModel)
        #else
        cameraContent
            .onAppear {
            ...
            .alert("需要相機權限", isPresented: $showPermissionAlert) {
                ...
                Text("請在「設定 > 隱私權 > 相機」中允許此 App 使用相機")
            }
        #endif
    }
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    Canvas { ctx, size in
                        ...
                        ctx.blendMode = .clear
                        ctx.fill(Path(ellipseIn: CGRect(x: cx - diameter / 2,
                                                        y: cy - diameter / 2,
                    ...
                    ForEach(Array(textRegions.enumerated()), id: \.offset) { _, rect in
                        let flipped = CGRect(x: rect.minX * geo.size.width,
        ...
        if case .processing = viewModel.state {
            ...
            Button {
                camera.capturePhoto { image in
                    Task { await viewModel.processCapture(image) }
                }
            } label: {
                ZStack {
                    Circle()
...
final class CameraController: NSObject, ObservableObject,
    ...
    func captureOutput(_ output: AVCaptureOutput,
        ...
        try? VNImageRequestHandler(cvPixelBuffer: pixelBuffer,
                                   orientation: .right, options: [:]).perform([req])
    }
}
